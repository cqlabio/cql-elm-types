...
*/
type DateTimePrecision = "Year" | "Month" | "Week" | "Day" | "Hour" | "Minute" | "Second" | "Millisecond"
type SortDirection = "asc" | "ascending" | "desc" | "descending"

type Element = TypeSpecifier |TupleElementDefinition |Expression |ExpressionDef |FunctionDef |ParameterDef |OperandDef |CaseItem |AliasedQuerySource |LetClause |RelationshipClause |SortByItem |SortClause |ReturnClause |AggregateClause |IncludeElement |CodeSystemDef |ValueSetDef |CodeDef |ConceptDef |UsingDef |IncludeDef |ContextDef |Library
type TypeSpecifier = NamedTypeSpecifier |IntervalTypeSpecifier |ListTypeSpecifier |TupleTypeSpecifier |ChoiceTypeSpecifier
type Expression = OperatorExpression |ExpressionRef |FunctionRef |ParameterRef |OperandRef |IdentifierRef |Literal |Tuple |Instance |Interval |List |If |Case |Null |MinValue |MaxValue |Filter |Sort |ForEach |Repeat |Current |Iteration |Total |AggregateExpression |Property |Query |AliasRef |QueryLetRef |Retrieve |CodeSystemRef |ValueSetRef |CodeRef |ConceptRef |Code |Concept |Quantity |Ratio
type OperatorExpression = UnaryExpression |BinaryExpression |TernaryExpression |NaryExpression |Round |Combine |Split |SplitOnMatches |PositionOf |LastPositionOf |Substring |TimeOfDay |Today |Now |Date |DateTime |Time |First |Last |Slice |IndexOf |Children |Descendents |Message |InCodeSystem |AnyInCodeSystem |InValueSet |AnyInValueSet
type BinaryExpression = And |Or |Xor |Implies |CanConvertQuantity |ConvertQuantity |Equal |Equivalent |NotEqual |Less |Greater |LessOrEqual |GreaterOrEqual |Add |Subtract |Multiply |Divide |TruncatedDivide |Modulo |Log |Power |LowBoundary |HighBoundary |Indexer |StartsWith |EndsWith |Matches |DurationBetween |DifferenceBetween |SameAs |SameOrBefore |SameOrAfter |Contains |ProperContains |In |ProperIn |Includes |IncludedIn |ProperIncludes |ProperIncludedIn |Before |After |Meets |MeetsBefore |MeetsAfter |Overlaps |OverlapsBefore |OverlapsAfter |Starts |Ends |Collapse |Expand |Times |Subsumes |SubsumedBy |CalculateAgeAt
type UnaryExpression = Not |IsNull |IsTrue |IsFalse |Is |As |Convert |CanConvert |ToBoolean |ConvertsToBoolean |ToConcept |ConvertsToDate |ToDate |ConvertsToDateTime |ToDateTime |ConvertsToDecimal |ToDecimal |ConvertsToInteger |ToInteger |ConvertsToLong |ToLong |ConvertsToQuantity |ToQuantity |ConvertsToRatio |ToRatio |ToList |ToChars |ConvertsToString |ToString |ConvertsToTime |ToTime |Ceiling |Floor |Truncate |Abs |Negate |Ln |Exp |Successor |Predecessor |Precision |Length |Upper |Lower |DateFrom |TimeFrom |TimezoneFrom |TimezoneOffsetFrom |DateTimeComponentFrom |PointFrom |Width |Size |Start |End |Exists |Flatten |Distinct |SingletonFrom |CalculateAge
type NaryExpression = Coalesce |Concatenate |Union |Intersect |Except
type TernaryExpression = ReplaceMatches
type AggregateExpression = Aggregate |Count |Sum |Product |Min |Max |Avg |GeometricMean |Median |Mode |Variance |PopulationVariance |StdDev |PopulationStdDev |AllTrue |AnyTrue
